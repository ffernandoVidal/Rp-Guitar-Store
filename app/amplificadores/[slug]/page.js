...
export default function DetalleAmplificador() {
  const params = useParams()
  const [amplificador, setAmplificador] = useState(null)
          ...
          <div className={styles.grid}>
            ...
            <div className={styles.infoSection}>
              ...
              <div className={styles.priceSection}>
                <span className={styles.price}>Q {amplificador.precio.toLocaleString('es-GT')}</span>
                <span className={styles.priceSuffix}>.00</span>
              </div>

              <div className={styles.description}>
                <h2 className={styles.sectionTitle}>Descripción</h2>
                <p>{amplificador.descripcion}</p>
              </div>

              {amplificador.caracteristicas && amplificador.caracteristicas.length > 0 && (
                <div className={styles.caracteristicas}>
                  <h2 className={styles.sectionTitle}>Características</h2>
                  <ul className={styles.caracteristicasList}>
                    {amplificador.caracteristicas.map((car, index) => (
                      <li key={index}>{car}</li>
                    ))}
                  </ul>
                </div>
